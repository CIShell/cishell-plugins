...
public class CSVtoDBAlgorithm implements Algorithm  {
	...
	public Data[] execute() throws AlgorithmExecutionException {
		...
		Database emptyDb = dbService.createNewDatabase();
		Database nsfDBInProgress = emptyDb;	
	...
	private void createNsfDbTables(Connection dbConnection, int dbID)
		...
		String createAwardTableSQL = "" + "CREATE TABLE " + NsfDatabaseFieldNames.AWARD_TABLE_NAME + dbID + 
		"(" + 
		NsfDatabaseFieldNames.AWARD_NUMBER + " INTEGER," +
		NsfDatabaseFieldNames.AWARD_HUMAN_READABLE_NAME + " VARCHAR(500)," +
		NsfDatabaseFieldNames.START_DATE + " DATE," + 
		NsfDatabaseFieldNames.EXPIRATION_DATE + " DATE," +
		NsfDatabaseFieldNames.AWARDED_AMOUNT_TO_DATE + " INTEGER," + 
		"PRIMARY KEY (" + NsfDatabaseFieldNames.AWARD_NUMBER + ")" + 
		")";
	...
	private void fillNsfDbQuickly(Connection nsfDbConnection, int dbID, CSVReader nsfCSVReader,
		...
		int awardNumberIndex = columnNameToColumnIndex.get(NsfCsvFieldNames.CSV.AWARD_NUMBER);
		int awardTitleIndex = columnNameToColumnIndex.get(NsfCsvFieldNames.CSV.AWARD_TITLE);
		int startDateIndex = columnNameToColumnIndex.get(NsfCsvFieldNames.CSV.AWARD_START_DATE);
		int expirationDateIndex = columnNameToColumnIndex.get(NsfCsvFieldNames.CSV.AWARD_EXPIRATION_DATE);
		int awardedAmountToDateIndex = columnNameToColumnIndex.get(NsfCsvFieldNames.CSV.AWARDED_AMOUNT_TO_DATE);
		
		String insertIntoAwardSql = "INSERT INTO " + NsfDatabaseFieldNames.AWARD_TABLE_NAME + dbID + 
		" (" + 
		NsfDatabaseFieldNames.AWARD_NUMBER + "," +
		NsfDatabaseFieldNames.AWARD_HUMAN_READABLE_NAME + "," +
		NsfDatabaseFieldNames.START_DATE + "," + 
		NsfDatabaseFieldNames.EXPIRATION_DATE + "," +
		NsfDatabaseFieldNames.AWARDED_AMOUNT_TO_DATE + 
		") " +
		"VALUES " 
		+ "("
		+ "?," //awardNumber
	...
	private Data annotateAndWrapAsData(Database nsfDB) {
